/*
 * $Log: mission.c,v $
 * Revision 1.15  2019/02/12 18:43:00  Saavick
 * Fixed item icon interpolation.
 * Fixed items not being detected in find/return-item missions with certain mission descriptions.
 *
...
//			puDoMethod( puGetObjectFromCollection( _pData->pCol, ROOTOBJ ), PUM_CONTROL_RECALC, 0, 0 );
	...
	*( PUU32* )( &CoordY ) = TempVal;
	...
	// Return item and find item missions SHOULD have had an item in the description, but we didn't find it.
	else if (TempVal == 0x2c41 || TempVal == 0x2c49)
	{
		WriteLog("find\tERROR, didn't find item in mission description: %s\n", pDesc);
		puSetAttribute( puGetObjectFromCollection( _pData->pCol, FINDITEM ), PUA_TEXTENTRY_BUFFER, 0 );
	}
	else
		puSetAttribute( puGetObjectFromCollection( _pData->pCol, FINDITEM ), PUA_TEXTENTRY_BUFFER, 0 );

	WriteLog("\n"); // Add an extra \n to seperate each mission.
...
static udtFindName_struc a_udtFindName[CNT_FINDNAME] = {
	...
	"simply, eh, clean it out.  The ", " is set to blow in 48 hours.  ",
	"  In this case the ", " is missing.  It must not fall into enemy hands.  ",
	" inside the a building, you will find the ", ".  Please bring it to us within 48 hours, ",
	" The enemy is currently making a new prototype ", ".  It is important that we get our hands on it as soon as possible.",
	", you will find the ", ".  Please bring it to us within 48 hours,",
	".  If you would find the ", " in there, we could track the criminal down!  ",
	" The enemy is flooding the market with forged ", " to undermine a very lucrative art series that we produce.  ",
	"  According to our sources, a ", " found in ",
	" If we can steal a ", " from our enemies, ",
	"  Fight your way inside, pick up the ", ", and destroy it safely ",
	" we have sure indications that the stolen ", " can be found somewhere ",
	" you should be able to find the ", ".  Please bring it back so we can ",
	" An extremely unstable ", " can be found in the enemy position ",
	" In there somewhere, you will find the ", ". It should contain something ",
	" credits to get me that ", "! You right-click the item on this mission booth, ",
	" obtain the prototype ", " in there, we would pay you ",
	" *BEEEEP*  Find prototype ", "!! Area ",
	" There is this ", ", it is a disaster waiting to happen. "
};
